...
#SE 로딩 변수
se_barrage  = None # 탄막발사시 SE로딩 변수
se_damage = None # 데미지를 입을 시 사용할 SE 로딩 변수
se_explosion = None #  보스 기체 폭발시 사용할 SE 로딩 변수
se_shot = None # 탄환 발사시 사용할 SE 로딩 변수
...
emy_shield = [0] * ENEMY_MAX # 적실드 리스트
emy_count = [0] * ENEMY_MAX # 적 움직임등을 관리할 리스트

EMY_BUULLET = 0 # 적의 탄환 번호 관리 상수
EMY_ZAKO = 1 # 일반 적 기체 번호 관리 리스트
EMY_BOSS = 5
...
def bring_enemy(): #적기체 등장
    sec = tmr / 30
    if sec > 0 and sec < 15 and tmr % 60 == 0:
...
def set_enemy(x, y, a, ty, sp, sh): #적기체 설정
    global  emy_no
    while True :
        ...
        if emy_f[i] == True:
            ...
            if emy_type[i] < EMY_BOSS: # 적 일반 기체 이동
                ...
                if emy_count[i] == 0 :
                    ...
                    if emy_x[i] < 200:
                        for j in range(10):
                            set_enemy(emy_x[i], emy_y[i]+80, j*20, EMY_BUULLET, 6, 0)
                        emy_count[i] = 2
                else:
                    emy_x[i] += emy_speed[i]
                    if emy_x[i] > 760:
                ...
                if emy_shield[i] < 100 and tmr % 30 == 0:
                    set_enemy(emy_x[i], emy_y[i]+80, random.randint(60,120), EMY_BUULLET, 6,0 )
